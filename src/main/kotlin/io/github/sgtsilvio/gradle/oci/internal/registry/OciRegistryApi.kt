...
//    ): Mono<Nothing> = deleteManifest(registryUrl, imageName, digest.toString(), credentials)
    ...
    private fun tryAuthorize(
        ...
        // Scopes that do not follow the "type:name:actions" format are ignored, as they carry no usable information.
        // The AWS ECR Public registry, for example, always responds with scope="aws", regardless of the requested resource.
        val scopesFromResponse = scope.split(' ').mapNotNullTo(HashSet()) { it.decodeToResourceScopeOrNull() }
        // If the response contains no usable scope, the scopes required for the request are used instead.
        val requestScopes = scopesFromResponse.ifEmpty { scopes }
        ...
        return tokenCache.getMono(TokenCacheKey(registryUrl, scopes, credentials?.hashed())) { key ->
            val scopeParams = requestScopes.joinToString("&scope=", "scope=") { it.encodeToString() }
            httpClient.headers { headers ->
                if (credentials != null) {
