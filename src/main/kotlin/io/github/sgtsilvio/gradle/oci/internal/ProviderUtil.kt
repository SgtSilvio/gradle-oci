...
fun <T, B, R> Provider<T>.zipAbsentAsEmptySet(other: Provider<Set<B>>, combiner: (T, Set<B>) -> R) =
    zip(other.orElse(setOf())) { t, b -> combiner.invoke(t, b) }

fun <T, K, V, R> Provider<T>.zipAbsentAsEmptyMap(other: Provider<Map<K, V>>, combiner: (T, Map<K, V>) -> R) =
